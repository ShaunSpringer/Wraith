...
(function() {
  ...
  this.Wraith = {
    ...
    compile: function(template) {
      ...
      fn = new Function('obj', 'var p=[],print=function(){p.push.apply(p,arguments);};' + 'with(obj||{}){p.push(\'' + template.replace(/\r/g, '\\r').replace(/\n/g, '\\n').replace(/\t/g, '\\t').replace(endMatch, "✄").split("'").join("\\'").split("✄").join("'").replace(c.interpolate, "' + ((hasOwnProperty('get') && get(\'$1\')) || $1) + '").replace(c.checked, "' + ((hasOwnProperty('get') && get(\'$1\') === true) ? 'checked' : \'\') + '").replace(c["class"], "class=\"' + ((hasOwnProperty('get') && get(\'$2\') === true) ? '$1' : \'\') + '\"").split(c.start).join("');").split(c.end).join("p.push('") + "');}return p.join('');");
      return fn;
    },
