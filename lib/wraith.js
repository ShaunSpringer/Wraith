...
(function() {
  ...
  this.Wraith.View = (function(_super) {
    ...
    View.prototype.createView = function(model, map) {
      var $view, Template, replace, template,
        _this = this;
      if (Wraith.DEBUG) {
        console.log('@Wraith.View', 'createView');
      }
      if (!($view = map.$view)) {
        return;
      }
      ...
      (function(model, map) {
        return model.bind('change', function(model_) {
          return _this.updateView(model, map, replace);
        });
      })(model, map);
      return this.updateView(model, map, replace);
    };

    View.prototype.updateView = function(model, map, replace) {
      var $view, $view_, Template, template;
      if (replace == null) {
        replace = false;
      }
      if (Wraith.DEBUG) {
        console.log('@Wraith.View', 'updateView');
      }
      if (!($view = map.$view)) {
        return;
      }
    ...
    View.prototype.removeView = function(model, map) {
      var $view;
      if (Wraith.DEBUG) {
        console.log('@Wraith.View', 'removeView');
      }
  ...
  this.Wraith.Controller = (function(_super) {
    ...
    Controller.prototype.loadViews = function() {
      ...
      return this.$el.find('[data-map]').forEach(function(item) {
        var $view, map, model_map, template, u, view;
        $view = $(item);
    ...
    Controller.prototype.registerModel = function(name, model) {
      ...
      _fn = function(model, map, view) {
        var isCollection;
        isCollection = mapping !== '' && model.get(mapping) instanceof Wraith.Collection;
        if (mapping !== '' && isCollection && map.$view[0].hasAttribute('data-repeat')) {
          ...
          return model.bind('change', function(model_) {
            return view.updateView(model_, map);
          });
        }
      };
      for (view_id in _ref) {
        map = _ref[view_id];
        if (!(map.model_map.slice(0, +(l - 1) + 1 || 9e9).toLowerCase() === nl)) {
          continue;
        }
