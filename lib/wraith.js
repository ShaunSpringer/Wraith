...
(function() {
  ...
  Wraith = (function() {
    ...
    Wraith.UIEvents = ['click', 'dblclick', 'mousedown', 'mouseup', 'mousemove', 'scroll', 'keypress', 'keyup', 'keydown', 'change', 'blur', 'focus'];
  ...
  Wraith.BaseView = (function(_super) {
    ...
    function BaseView($el) {
      ...
      this.bindClass = __bind(this.bindClass, this);
      this.bindClasses = __bind(this.bindClasses, this);
    ...
    BaseView.prototype.bindClasses = function($view, model) {
      var $el, els, _i, _len;

      if ($view.attributes['data-class']) {
        this.bindClass($view, model);
      }
      els = $view.querySelectorAll('[data-class]');
      for (_i = 0, _len = els.length; _i < _len; _i++) {
        $el = els[_i];
        this.bindClass($el, model);
      }
      return this;
    };

    BaseView.prototype.bindClass = function($view, model) {
      var binding, invert, klass, klassMap, klasses, results, tokens, _i, _len, _ref, _ref1, _results;
  ...
  Wraith.ViewModel = (function(_super) {
    ...
    ViewModel.TEMPLATE_REGEX = new RegExp(Wraith.templateSettings.start + '\\s*(' + Wraith.templateSettings.dotNotation + ')\\s*' + Wraith.templateSettings.end, 'gi');
    ...
    ViewModel.prototype.render = function(model) {
      ...
      $el.innerHTML = this.compileTemplate(model, this.template);
      $el = $el.firstChild;
      return $el;
    };
  ...
  Wraith.CollectionView = (function(_super) {
    ...
    CollectionView.prototype.createView = function(model) {
      ...
      this.bindClasses($el, model);
      return this.bindUIEvents($el);
    };
    ...
    CollectionView.prototype.updateView = function(model) {
      var $el, $node, $oldView, id;

      if (!($oldView = this.$parent.querySelector('[data-model=' + model.get('_id') + ']'))) {
        return;
      }
      this.unbindUIEvents($oldView);
      $el = this.render(model);
      if (!($node = $el)) {
        return;
      }
      id = $oldView.attributes['data-id'].value || Wraith.uniqueId();
      $node.setAttribute('data-id', id);
      $node.setAttribute('data-model', model.get('_id'));
      this.$parent.replaceChild($node, $oldView);
      this.bindClasses($node, model);
      return this.bindUIEvents($node);
    };
