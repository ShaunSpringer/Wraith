...
(function() {
  ...
  this.Wraith.View = (function(_super) {
    ...
    View.prototype.updateView = function(model) {
      ...
      this.$parent.replaceChild($el, this.$el);
      return this.$el = $el;
    };
  ...
  this.Wraith.RepeatingView = (function(_super) {
    ...
    function RepeatingView($parent, template) {
      var _base, _name, _ref;
      this.$parent = $parent;
      this.template = template;
      if (Wraith.DEBUG) {
    ...
    RepeatingView.prototype.createView = function(model) {
      ...
      if ($node = $el.firstChild) {
        ...
        return this.$parent.appendChild($el.firstChild);
      }
    };

    RepeatingView.prototype.updateView = function(model) {
      var $el, $view;
      if (!($view = this.$parent.querySelector('[data-id=' + model.get('_id') + ']'))) {
        return;
      }
      $el = this.render(model);
      return this.$parent.replaceChild($el, $view);
    };
  ...
  this.Wraith.Controller = (function(_super) {
    ...
    Controller.prototype.bindViews = function(name, model) {
      ...
      for (_i = 0, _len = bindings.length; _i < _len; _i++) {
        map = bindings[_i];
        this.bindView(model, map);
      }
      return this;
    };

    Controller.prototype.bindView = function(model, map) {
      var binding, mapping, view;
      binding = map.binding;
      view = map.view;
